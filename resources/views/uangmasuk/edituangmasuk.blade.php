...
@section('content')
    ...
    <body>
        <h1 class="text-center">Edit Uang Masuk</h1>
        <div class="mt-3">
            <form action="/updateuangmasuk/{{ $uangmasuk->id }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    ...
                    <select class="ipan_select3 form-control select2" style="width:100%;" name="penabungs_id" class=""
                        ...
                        @foreach ($penabung as $item)
                            <option value="{{ $item->id }}"<?php if ($uangmasuk->penabungs_id == $item->id) {
                                echo 'selected';
                            } ?>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" readonly name="jenis_kelamin"
                        value="{{ $uangmasuk->jenis_kelamin }}" id="jenis_kelamin">
                </div>
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">Alamat</label>
                    <input type="text" class="form-control" readonly name="alamat" value="{{ $uangmasuk->alamat }}"
                        id="alamat">
                </div>
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">No Telpon</label>
                    <input type="number" class="form-control" readonly name="notelpon" value="{{ $uangmasuk->notelpon }}"
                        id="notelpon">
                </div>
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">Jumlah Uang Masuk</label>
                    <div class="input-group mb-3">
                        <span class="input-group-text" id="basic-addon1">Rp.</span>
                        <input type="text" class="form-control" name="jumlah_uang" value="{{ $uangmasuk->jumlah_uang }}">
                    </div>
                </div>
                <div class="mb-3">
