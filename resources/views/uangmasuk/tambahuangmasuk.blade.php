...
@section('content')
    ...
    <body>
        <h1 class="text-center">Tambah Uang Masuk</h1>
        <div class="mt-3">
            <form action="/insertuangmasuk" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <select class="ipan_select3 form-control select2" style="width:100%;" name="penabungs_id" class=""
                        ...
                        @foreach ($penabung as $item)
                            <option value="{{ $item->id }}" data-jenis_kelamin="{{ $item->jenis_kelamin }}"
                                data-alamat="{{ $item->alamat }}" data-notelpon="{{ $item->notelpon }}">
